...
from src.ui.video.video_window import VideoWindow
...
class Worker(QThread):
    ...
    def render(self, path):
        print("Rendering thread ", path)
        self.path = path
        self.start()

    def run(self):
        """Long-running task."""
        res = create_subtitles(self.path)
...
class WelcomeWindow(WindowInterface):
    ...
    def dropEvent(self, event):
        ...
        #     self.inputField.error_label.pop_error(
        #         "\tFile extension not supported\t\nSupported extensions are wav, mp4 and mp3")
        #     event.ignore()
        #
        # else:
    ...
    def thread_finished(self, arg):
        print("I am in thread_finished", arg)
        self.cams = VideoWindow(self.file_path, arg)
        self.cams.show()
        self.close()
